...
2. **本 skill 自带的脚本位于「本 skill 安装目录」内**，不是当前工作目录。调用时**必须用脚本的绝对路径**——本 skill 通常被安装/软链到 `<SKILL_DIR>/` 或 `<SKILL_DIR>/`。**禁止**直接 `bash scripts/check_env.sh`（当前工作目录通常没有 `scripts/`，会 No such file or directory）。正确做法：先确定本 SKILL.md 所在目录 `<SKILL_DIR>`，再执行 `bash <SKILL_DIR>/scripts/check_env.sh`。下文所有 `scripts/xxx` 均指相对本 skill 目录。
